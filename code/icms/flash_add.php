...
/*	if(empty($typeid) || ereg("[^0-9]", $typeid))
	...
	*/
	$query = "Insert Into `#@__flash`(sortrank,url,webname,pic,msg,dtime,ischeck)
            Values('$sortrank','$url','$webname','$imgurl','$msg','$dtime','$ischeck'); ";
	$rs = $dsql->ExecuteNoneQuery($query);
	$burl = empty($_COOKIE['ENV_GOBACK_URL']) ? "flash_main.php" : $_COOKIE['ENV_GOBACK_URL'];
	if($rs)
	{
		ShowMsg("成功增加一个幻灯图片!",$burl,0,500);
		exit();
	}
	else
	{
		ShowMsg("增加幻灯图片时出错，请向官方反馈，原因：".$dsql->GetError(),"javascript:;");
		exit();
	}
}
