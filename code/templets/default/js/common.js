...
function scrollImg(animateTime, time) {
    ...
    function delay(e, n, time) {
        ...
        // console.log($('#scrollImgs .wrap .imgs').html());

        n.find("a").removeClass('hover');
        $(n[i]).find("a").addClass('hover');
    }

    function btnLR(direction,btn,time,img,e){
        if (direction=='L') {
            var i = GLOBAL.BANNER.MIAN.l = GLOBAL.BANNER.MIAN.l - 1;
           if (i <= -1) {
    ...
    btnL.click(function(){btnLR('L',btnL,animateTime,imgS,navA)});
    btnR.click(function(){btnLR('R',btnR,animateTime,imgS,navA)});
    // btnR.click(objBtn,function(){btnLR('R')}());
